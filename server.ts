// ============================================================
//  VILA REAL FUTSAL — API
//
//  Adaptado a partir da versão que rodava como Edge Function no
//  Supabase, para rodar de forma independente num servidor próprio
//  (Deno + Docker). A lógica de negócio é a mesma; a única mudança
//  estrutural é que as fotos/imagens agora vão para o MinIO
//  (armazenamento compatível com S3, self-hosted) em vez do
//  Supabase Storage.
// ============================================================
...
import { Client as MinioClient } from 'npm:minio@^8';

// ---------- Configuração / variáveis de ambiente ----------
const DB_URL = Deno.env.get('DATABASE_URL');
const JWT_SECRET = Deno.env.get('JWT_SECRET') || 'troque-este-segredo-no-.env';
...
const MINIO_ENDPOINT = Deno.env.get('MINIO_ENDPOINT') || 'minio';
const MINIO_PORT = parseInt(Deno.env.get('MINIO_PORT') || '9000');
const MINIO_ACCESS_KEY = Deno.env.get('MINIO_ACCESS_KEY')!;
const MINIO_SECRET_KEY = Deno.env.get('MINIO_SECRET_KEY')!;
// URL pública (via domínio/subdomínio no Nginx Proxy Manager) usada nos
// links de foto devolvidos ao navegador. Diferente do MINIO_ENDPOINT
// interno, que só é alcançável dentro da rede Docker.
const MINIO_PUBLIC_URL = (Deno.env.get('MINIO_PUBLIC_URL') || `http://${MINIO_ENDPOINT}:${MINIO_PORT}`).replace(/\/$/, '');
const FOTOS_BUCKET = 'fotos';

if (!DB_URL) console.warn('⚠️  DATABASE_URL não configurada.');
if (!APP_URL) console.warn('⚠️  APP_URL não configurada — links de e-mail podem ficar incorretos.');
if (!MINIO_ACCESS_KEY || !MINIO_SECRET_KEY) console.warn('⚠️  MINIO_ACCESS_KEY/MINIO_SECRET_KEY não configuradas.');
...
const DEFAULT_PERMISSIONS = {
  ...
  notificacoes: true, backup: false, configuracoes: false, usuarios: false, personalizar: false,
  excluirAlunos: false, habilitarDesabilitar: false,
};
...
function rateLimit(windowMs: number, max: number) {
  return (req: express.Request, res: express.Response, next: express.NextFunction) => {
    ...
    if (fresh.length >= max) return res.status(429).json({ error: 'Muitas tentativas. Aguarde alguns minutos e tente novamente.' });
    fresh.push(now);
...
async function ensureSchema() {
  ...
  await pool.query(`
    CREATE TABLE IF NOT EXISTS users (
      id TEXT PRIMARY KEY, nome TEXT NOT NULL, email TEXT UNIQUE NOT NULL, telefone TEXT,
      perfil TEXT NOT NULL, usuario TEXT, senha_hash TEXT NOT NULL,
      status TEXT NOT NULL DEFAULT 'aprovado', permissoes JSONB NOT NULL DEFAULT '{}',
      alunos_vinculados JSONB NOT NULL DEFAULT '[]', criado_em TIMESTAMPTZ NOT NULL DEFAULT now(),
      primeiro_acesso BOOLEAN NOT NULL DEFAULT true, reset_token_hash TEXT, reset_token_expires TIMESTAMPTZ
    );
  `);
  await pool.query(`
    CREATE TABLE IF NOT EXISTS app_state (
      id INTEGER PRIMARY KEY DEFAULT 1, data JSONB NOT NULL DEFAULT '{}', updated_at TIMESTAMPTZ NOT NULL DEFAULT now()
    );
  `);
...
// ---------- MinIO: bucket de fotos ----------
let bucketGarantido = false;
async function ensureBucket() {
  if (bucketGarantido) return;
  const existe = await minioClient.bucketExists(FOTOS_BUCKET).catch(() => false);
  if (!existe) await minioClient.makeBucket(FOTOS_BUCKET);
  const policy = {
    Version: '2012-10-17',
    Statement: [{ Effect: 'Allow', Principal: { AWS: ['*'] }, Action: ['s3:GetObject'], Resource: [`arn:aws:s3:::${FOTOS_BUCKET}/*`] }],
  };
  await minioClient.setBucketPolicy(FOTOS_BUCKET, JSON.stringify(policy));
...
async function uploadFotoBase64(dataUrl: string, pastaPrefixo: string): Promise<string> {
  ...
  await minioClient.putObject(FOTOS_BUCKET, nomeArquivo, bytes, bytes.length, { 'Content-Type': mime });
  return `${MINIO_PUBLIC_URL}/${FOTOS_BUCKET}/${nomeArquivo}`;
}
...
app.post('/api/auth/login', rateLimit(15 * 60 * 1000, 15), async (req, res) => {
  try {
    ...
    const result = await pool.query('SELECT * FROM users WHERE usuario=$1 OR email=$1', [String(usuario).trim()]);
...
app.post('/api/auth/register', rateLimit(15 * 60 * 1000, 10), async (req, res) => {
  ...
  try {
    ...
    notificacoes.push({ id: 'notif_' + Date.now(), tipo: 'novo_cadastro', usuarioNome: 'Sistema', mensagem: `Novo cadastro: ${nome} (${email}) — Pai/Responsável`, data: new Date().toISOString() });
    atual.notificacoes = notificacoes;
    const mensagens = Array.isArray(atual.mensagensInternas) ? atual.mensagensInternas : [];
    mensagens.push({ id: 'msg_' + Date.now(), remetenteId: 'admin', remetenteNome: 'Vila Real Futsal', geral: false, destinatarioId: id, destinatarioNome: nome, mensagem: 'Bem-vindo(a) ao Vila Real Futsal! 🏆⚽ Cadastre seu(s) aluno(s) para começar.', data: new Date().toISOString(), lidaPor: [] });
...
app.post('/api/auth/forgot-password', rateLimit(15 * 60 * 1000, 8), async (req: any, res: any) => {
  ...
  try {
    ...
    await enviarEmail({
      to: user.email, subject: 'Redefinição de senha — Vila Real Futsal',
      html: `<p>Olá, ${nomeSeguro}.</p><p>Recebemos uma solicitação para redefinir sua senha no sistema Vila Real Futsal.</p><p><a href="${link}" style="background:#d4a020;color:#0f1b2d;padding:10px 20px;border-radius:8px;text-decoration:none;font-weight:bold;">Redefinir minha senha</a></p><p>Ou copie e cole este link no navegador:<br>${link}</p><p>Este link é válido por 30 minutos.</p>`,
...
app.post('/api/auth/reset-password', rateLimit(15 * 60 * 1000, 10), async (req: any, res: any) => {
  try {
    ...
    const user = result.rows[0];
    if (!user || !user.reset_token_hash || !user.reset_token_expires) return res.status(400).json({ error: 'Link inválido ou expirado' });
    if (new Date(user.reset_token_expires) < new Date()) return res.status(400).json({ error: 'Link expirado. Solicite um novo.' });
...
app.get('/api/public/branding', async (_req: any, res: any) => {
  try {
    ...
    const aparencia: any = {};
    CAMPOS_APARENCIA_PUBLICOS.forEach((campo) => { if (config[campo] !== undefined) aparencia[campo] = config[campo]; });
...
app.put('/api/auth/admin-profile', auth, async (req: any, res: any) => {
  if (req.dbUser.id !== 'admin' || req.dbUser.perfil !== 'Administrador') return res.status(403).json({ error: 'Apenas o administrador principal' });
  try {
...
app.post('/api/auth/vincular-aluno', auth, async (req: any, res: any) => {
  try {
    if (req.dbUser.perfil !== 'Pai') return res.status(403).json({ error: 'Apenas Pais/Responsáveis podem se autovincular a um aluno' });
    const { alunoId } = req.body || {};
    if (!alunoId) return res.status(400).json({ error: 'Informe o aluno' });
    const state = await pool.query('SELECT data FROM app_state WHERE id=1');
    const alunos = state.rows[0]?.data?.alunos || [];
    if (!alunos.some((a: any) => a && a.id === alunoId)) return res.status(404).json({ error: 'Aluno não encontrado' });
    const vinculadosAtuais = Array.isArray(req.dbUser.alunos_vinculados) ? req.dbUser.alunos_vinculados : [];
    if (vinculadosAtuais.includes(alunoId)) return res.json({ ok: true, alunosVinculados: vinculadosAtuais });
    if (vinculadosAtuais.length >= 2) return res.status(400).json({ error: 'Limite de 2 alunos por Pai/Responsável excedido' });
...
app.post('/api/users/:id/toggle-status', auth, async (req: any, res: any) => {
  try {
    const podeUsar = req.dbUser.perfil === 'Administrador' || (req.dbUser.perfil === 'Professor' && req.dbUser.permissoes && req.dbUser.permissoes.habilitarDesabilitar === true);
...
app.put('/api/state', auth, async (req: any, res: any) => {
  ...
  try {
    ...
    if (!isAdminUser) { delete incoming.usuarios; if (incoming.config) delete incoming.config.senhaMaster; }
    ...
    if (Array.isArray(incoming.alunos)) {
      for (const a of incoming.alunos) {
        if (a && typeof a.foto === 'string' && a.foto.startsWith('data:image/')) a.foto = await uploadFotoBase64(a.foto, 'aluno');
      }
    }
...
// Rota pública (sem autenticação) — registra o clique num anúncio/patrocinador
// do carrossel. O front dispara isso em segundo plano e ignora falhas, então
// aqui também respondemos rápido e não deixamos erro nenhum vazar como 500.
app.post('/api/public/registrar-clique', rateLimit(60 * 1000, 30), async (req: any, res: any) => {
  try {
    const { tipo, anuncioId } = req.body || {};
    if (tipo && anuncioId) {
      await pool.query('INSERT INTO cliques_anuncios (tipo, anuncio_id) VALUES ($1,$2)', [String(tipo), String(anuncioId)]);
    }
    res.status(204).end();
  } catch (e) {
    console.error('Erro ao registrar clique:', e);
    res.status(204).end(); // nunca falha de forma visível para o visitante
  }
});

app.use('/api', (_req: any, res: any) => res.status(404).json({ error: 'Rota não encontrada' }));

app.listen(8000, () => console.log('🏆 Vila Real Futsal API rodando em http://0.0.0.0:8000'));
